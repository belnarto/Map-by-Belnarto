...
public class GraphAdjMatrix extends Graph {
    ...
    public List<Integer> getDistance2(int v) {
        ...
        int adjMatrixSize = adjMatrix.length;
        if (v < adjMatrixSize) {
            int[][] multiplicationResult = new int[adjMatrixSize][adjMatrixSize];
            for (int row = 0; row < adjMatrixSize; row++) {
                for (int column = 0; column < adjMatrixSize; column++) {
                    multiplicationResult[row][column] = 0;
                    for (int i = 0; i < adjMatrixSize; i++) {
                        multiplicationResult[row][column] += adjMatrix[row][i] * adjMatrix[i][column];
                    }
                }
            }
            for (int i = 0; i < adjMatrixSize; i++) {
                if (multiplicationResult[v][i] > 0) {
                    while (multiplicationResult[v][i] > 0) {
                        resultList.add(i);
                        multiplicationResult[v][i]--;
                    }
                }
            }
            return resultList;
